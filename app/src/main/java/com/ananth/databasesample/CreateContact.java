            ...
            if (!TextUtils.isEmpty(mEmail.getText().toString().trim())) {
                if (!TextUtils.isEmpty(mPhone.getText().toString().trim())) {
                    if (!TextUtils.isEmpty(mLocation.getText().toString().trim())) {
                        ...
                        if (type.equals("edit")) {
                            if (myDb.updateContacts(mName.getText().toString().trim(), mEmail.getText().toString().trim(), mPhone.getText().toString().trim(), mLocation.getText().toString().trim(), mSelectedFilePath.toString())) {
                                Utils.mUpdated = true;
                                startActivity(new Intent(CreateContact.this, ContactListActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK));
                                finish();
                            } else {
                                Toast.makeText(getApplicationContext(), "Could not update!",
                                        Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Boolean mPhoneExist = myDb.getPhone(mPhone.getText().toString());
                            if (mPhoneExist) {
                                Toast.makeText(getApplicationContext(), "Contact already exist!",
                                        Toast.LENGTH_SHORT).show();
                            } else {
                                if (myDb.insertContacts(mName.getText().toString().trim(), mEmail.getText().toString().trim(), mPhone.getText().toString().trim(), mLocation.getText().toString().trim(), mSelectedFilePath.toString())) {
                                    Utils.mInserted = true;
                                    finish();
                                } else {
                                    Toast.makeText(getApplicationContext(), "Could not create!",
                                            Toast.LENGTH_SHORT).show();
                                }
                            }
                        }
